...
select save_shift(null,:'a','2026-09-23','manana', p_request_id := gen_random_uuid()) as manana \gset
select save_shift(null,:'a','2026-09-23','tarde', p_request_id := gen_random_uuid()) as tarde \gset
select save_shift(null,:'a','2026-09-23','noche', p_request_id := gen_random_uuid()) as noche \gset
select lives_ok(format($$select save_shift(null,%L,'2026-09-24','manana', p_request_id := gen_random_uuid())$$, :'a'),
 'límites contiguos no se superponen');
select throws_ok(format($$select save_shift(null,%L,'2026-09-23','guardia',null,'10:00', p_request_id := gen_random_uuid())$$, :'a'),
 '23P01',null,'guardia choca con franjas distintas');
select throws_ok(format($$select save_shift(null,%L,'2026-09-24','franco', p_request_id := gen_random_uuid())$$, :'a'),
 '23P01',null,'franco choca con la noche anterior');
select throws_ok(format($$select save_shift(null,%L,'2026-09-25','guardia', p_request_id := gen_random_uuid())$$, :'a'),
 '23514',null,'guardia exige horario explícito');
select save_shift(null,:'b','2026-09-23','manana', p_request_id := gen_random_uuid()) as ocupado \gset
...
select throws_ok(format($$select save_shift(null,%L,'2026-09-23','manana', p_request_id := gen_random_uuid())$$, :'c'),
...
select lives_ok(format($$select save_shift(null,%L,'2026-09-23','manana', p_request_id := gen_random_uuid())$$, :'c'),
...
select save_shift(null,:'b','2026-09-25','guardia',null,'19:00', p_request_id := gen_random_uuid()) as guardia \gset
select throws_ok(format($$select save_shift(null,%L,'2026-09-26','guardia',null,'06:00', p_request_id := gen_random_uuid())$$, :'b'),
 '23P01',null,'dos guardias en días distintos también pueden superponerse');
select lives_ok(format($$select save_shift(null,%L,'2026-09-26','manana', p_request_id := gen_random_uuid())$$, :'b'),
 'guardia nocturna termina a las 07:00 del día siguiente');
select save_shift(null,:'c','2026-09-27','franco', p_request_id := gen_random_uuid()) as franco \gset
