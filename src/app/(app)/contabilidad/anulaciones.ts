...
export async function anularPago(
  ...
  try {
    ...
    if (!cuota)
      return {
        motivo,
        error: "La cuota no pertenece a esta cuenta o no está disponible.",
      };
    const pago = await obtenerMovimiento(cuotaId, pagoId);
    if (!pago || pago.voided_at)
      return {
        motivo,
        error: "El pago ya fue anulado o no pertenece a esta cuota.",
      };
...
export async function cancelarCuota(
  ...
  try {
    const cuota = await obtenerCuota(admissionId, cuotaId);
    if (!cuota || cuota.payment_status === "cancelled")
      return {
        motivo,
        error: "La cuota ya está anulada o no pertenece a esta cuenta.",
      };
    if (cuota.paid_amount > 0)
      return {
        motivo,
        error:
          "La cuota tiene pagos vigentes. Revisalos y anulalos primero si corresponde.",
      };
    const supabase = await createClient();
    const { data, error } = await supabase.rpc("cancel_monthly_charge", {
      p_monthly_charge_id: cuotaId,
      p_reason: motivo,
    });
    if (error || !data) return { motivo, ...errorAnulacion(error) };
  } catch (error) {
