...
function datos(): FormData {
  ...
  form.set(
    "comprobante",
    new File(["%PDF-1.7"], "apellido-privado.pdf", { type: "application/pdf" }),
  );
  return form;
}
beforeEach(() => {
...
it("sube con nombre generado y vincula la ruta al pago solo tras subir", async () => {
  await expect(registrarPago("estadia", "cuota", inicial, datos())).rejects.toThrow(
    "REDIRECT",
  );
  ...
  expect(mocks.rpc).toHaveBeenCalledWith(
    "record_payment",
    expect.objectContaining({ p_receipt_path: ruta }),
  );
...
it.each(["rechazo", "interrupcion"])("no cobra si falla la carga: %s", async caso => {
  if (caso === "rechazo")
    mocks.upload.mockResolvedValue({ error: { message: "detalle privado" } });
...
it("no sube ni cobra un archivo con firma inválida", async () => {
  const form = datos();
  form.set("comprobante", new File(["falso"], "foto.jpg", { type: "image/jpeg" }));
  expect(
    (await registrarPago("estadia", "cuota", inicial, form)).errores.comprobante,
  ).toBeTruthy();
...
it("conserva el archivo y bloquea el reenvío cuando el pago tiene resultado incierto", async () => {
  ...
  expect((await registrarPago("estadia", "cuota", inicial, datos())).bloqueado).toBe(
    true,
  );
