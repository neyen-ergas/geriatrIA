...
export function FormularioCarga({
  ...
  const errorCampo = (campo: string) =>
    estado.errores[campo] && (
      <p id={`${campo}-error`} className="mt-1 text-sm text-red-700">
        {estado.errores[campo]}
      </p>
    );
  return (
    <form action={action} className="mt-6 max-w-2xl space-y-4" noValidate>
      ...
      <Card className="p-6">
        <fieldset disabled={pendiente || estado.bloqueado} className="space-y-5">
          <legend className="sr-only">
            {modo === "cuota" ? "Datos de la cuota" : "Datos del pago"}
          </legend>
          {modo === "cuota" ? (
            <>
              <div>
                ...
                <Input
                  ...
                  onChange={evento => {
                    ...
                    setVencimiento(
                      vencimientoSugerido(evento.target.value, diaVencimiento),
                    );
                  }}
                />
                {errorCampo("periodo")}
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              {modo === "cuota"
                ? "Confirmá o ajustá el importe sugerido. "
            ...
            <>
              <div>
                ...
                <select
                  {...atributos("medio")}
                  required
                  defaultValue={estado.valores.medio}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={evento => {
                    ...
                    setErrorArchivo(
                      archivo && archivo.size > MAX_COMPROBANTE_BYTES
                ...
                />
                <p className="mt-1 text-xs text-slate-500">
                  JPG, PNG o PDF, hasta 3 MB. Se guarda de forma privada. Si el envío
                  falla, seleccioná el archivo nuevamente.
                </p>
                {errorArchivo && (
          ...
          <div>
            <Label htmlFor="notas">
              Observaciones{" "}
              {modo === "pago" ? "(obligatorias para otro medio)" : "(opcional)"}
            </Label>
      ...
      <Link
        href={volver}
        className="inline-block py-2 text-sm font-medium text-sky-700 hover:underline"
      >
        Volver a la cuenta
      </Link>
    </form>
