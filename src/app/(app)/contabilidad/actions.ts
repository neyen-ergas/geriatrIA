...
export async function crearCuota(
  ...
  try {
    ...
    if (!cuenta)
      return { valores, errores: {}, mensaje: "La estadía no está disponible." };
  ...
  try {
    ...
    if (!cuota)
      return {
        valores,
        errores: {},
        mensaje: "La cuota no pertenece a esta cuenta o no está disponible.",
      };
    ...
    const supabase = await createClient();
    const comprobante = await cargarComprobante(
      supabase,
      admissionId,
      cuotaId,
    ...
    if (!comprobante.ok)
      return {
        valores,
        errores: { comprobante: comprobante.error },
