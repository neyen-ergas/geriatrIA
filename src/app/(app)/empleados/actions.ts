...
export async function guardarEmpleado(
  ...
  try {
    ...
    return { valores, errores: {}, mensaje: errorEmpleado(error) };
  }
...
export async function darBajaEmpleado(
  ...
  try {
    ...
    if (!empleado || empleado.terminated_at)
      return {
        valores,
        errores: {},
        mensaje:
          "La ficha no está disponible para dar de baja. Volvé al listado para revisarla.",
      };
    ...
    const { data, error } = await supabase.rpc("terminate_employee", {
      ...
      p_terminated_at: valores.terminated_at,
      p_reason: valores.termination_reason,
    });
    if (error || !data) return { valores, errores: {}, mensaje: errorEmpleado(error) };
  } catch (error) {
    return { valores, errores: {}, mensaje: errorEmpleado(error) };
