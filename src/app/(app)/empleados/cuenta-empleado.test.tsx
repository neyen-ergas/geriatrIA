...
const cuenta: Acceso = {
  user_id: "cuenta", email: "ficticio@example.invalid", role: "management", enabled: false,
  updated_at: "2026-09-14T12:00:00.123456Z", employee_id: null, employee_name: null, employee_terminated_at: null,
};
it("solo ofrece cuentas con perfil y sin otro vínculo", () => {
  const html = renderToStaticMarkup(<CuentaEmpleado empleadoId="empleado" inactivo={false} cuentas={[
    cuenta,
    { ...cuenta, user_id: "ocupada", email: "ocupada@example.invalid", employee_id: "otro" },
    { ...cuenta, user_id: "pendiente", email: "pendiente@example.invalid", role: null, updated_at: null },
  ]} />);
...
it("con vínculo habilitado indica suspender antes de la baja y permite corregirlo", () => {
  const html = renderToStaticMarkup(<CuentaEmpleado empleadoId="empleado" inactivo={false} cuentas={[{ ...cuenta, employee_id: "empleado", enabled: true }]} />);
...
it("una baja sin vínculo no ofrece nuevas asignaciones", () => {
  const html = renderToStaticMarkup(<CuentaEmpleado empleadoId="empleado" inactivo cuentas={[cuenta]} />);
...
it("Accesos enlaza a la ficha e impide elegir habilitado para una baja", () => {
  const html = renderToStaticMarkup(<FormularioAcceso acceso={{ ...cuenta, employee_id: "empleado", employee_name: "Persona <script>", employee_terminated_at: "2025-02-01" }} />);
