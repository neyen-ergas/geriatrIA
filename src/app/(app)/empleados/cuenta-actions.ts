...
async function guardarVinculo(
  ...
  if (
    !esIdCuenta(empleadoId) ||
    !esIdCuenta(usuarioId) ||
    typeof version !== "string" ||
    !Number.isFinite(Date.parse(version))
  ) {
    return {
      error: "Elegí una cuenta con perfil asignado. Si no aparece, revisá Accesos.",
      ok: false,
    };
  }
  try {
    ...
    return { error: errorVinculo(error), ok: false };
  }
