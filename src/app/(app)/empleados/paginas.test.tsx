...
vi.mock("@/lib/empleados-datos", () => ({
  listarEmpleados: mocks.listar,
  obtenerEmpleado: mocks.ficha,
}));
...
it.each([
  ...
  () => EmpleadoPage(parametros),
  () => EditarEmpleadoPage(parametros),
  () => BajaEmpleadoPage(parametros),
  () => NuevoEmpleadoPage(),
...
it("la ficha dada de baja conserva motivo sin ofrecer escrituras", async () => {
  ...
  expect(html).not.toContain("Editar ficha");
