...
export default async function EmpleadoPage({
  ...
  const campos = [
    ["DNI", empleado.dni],
    ["Puesto", empleado.job_title],
    ["Fecha de alta", formatearFechaPago(empleado.hired_at)],
    [
      "Nacimiento",
      empleado.birth_date ? formatearFechaPago(empleado.birth_date) : "Sin informar",
    ],
    ["Teléfono", empleado.phone || "Sin informar"],
    ["Correo", empleado.email || "Sin informar"],
    ["Observaciones", empleado.notes || "Sin observaciones"],
    ...(empleado.terminated_at
      ? [
          ["Fecha de baja", formatearFechaPago(empleado.terminated_at)],
          ["Motivo", empleado.termination_reason ?? ""],
        ]
      : []),
  ];
  return (
    <div className="mx-auto max-w-3xl">
      <Link
        href={empleado.terminated_at ? "/empleados?estado=bajas" : "/empleados"}
      ...
      <p className="mt-2 text-sm text-slate-600">
        {empleado.terminated_at ? "Baja · Ficha de consulta" : "Empleado activo"}
      </p>
        ...
        <dl className="grid gap-5 sm:grid-cols-2">
          {campos.map(([etiqueta, valor]) => (
            <div key={etiqueta}>
              ...
              <dd className="mt-1 whitespace-pre-wrap break-words text-sm">{valor}</dd>
            </div>
          ))}
        </dl>
      </Card>
      {!empleado.terminated_at && (
        <nav className="mt-5 flex gap-5 text-sm font-medium">
          <Link
          ...
          <Link
            href={`/empleados/${empleado.id}/baja`}
      ...
      <CuentaEmpleado
        ...
        inactivo={empleado.terminated_at !== null}
        cuentas={cuentas}
      />
    </div>
  );
