...
const mocks = vi.hoisted(() => ({ sesion: vi.fn(), cliente: vi.fn(), rpc: vi.fn(), empleado: vi.fn(), revalidar: vi.fn(), redirigir: vi.fn() }));
...
it.each(acciones)("exige sesión antes de cualquier lectura o escritura", async accion => {
  ...
  expect(mocks.cliente).not.toHaveBeenCalled(); expect(mocks.empleado).not.toHaveBeenCalled();
});
...
it("conserva los microsegundos de la versión", async () => {
  ...
  expect(mocks.rpc).toHaveBeenCalledWith("save_employee", expect.objectContaining({ p_expected_updated_at: VERSION }));
});
it.each(["40001", "23505", "23514", "42501", "desconocido"])("no reintenta ni expone detalles al fallar (%s)", async code => {
...
function formulario(): FormData {
  ...
  for (const [campo, valor] of Object.entries({ first_name: "Persona ficticia", last_name: "Prueba", dni: "TEST-EMP", job_title: "Cuidador", hired_at: "2025-01-01", terminated_at: "2025-02-01", termination_reason: "Fin ficticio" })) datos.set(campo, valor);
  return datos;
}
