...
const inicial: EstadoFamiliar = {
  valores: leerFamiliar(new FormData()),
  errores: {},
  mensaje: null,
};
beforeEach(() => {
...
it("exige gestión antes de validar o escribir", async () => {
  ...
  await expect(
    guardarFamiliar(residente, contacto, version, inicial, formulario()),
...
it.each([null, version])(
  "alta/edición conserva id y versión precisa (%s)",
  async fecha => {
    await expect(
      guardarFamiliar(residente, contacto, fecha, inicial, formulario()),
    ).rejects.toThrow("REDIRECT");
    expect(mocks.rpc).toHaveBeenCalledExactlyOnceWith(
      "save_family_contact",
      expect.objectContaining({
        p_id: contacto,
        p_resident_id: residente,
        p_expected_updated_at: fecha ?? undefined,
        p_first_name: "Familiar",
        p_is_emergency_contact: true,
        p_is_payment_responsible: false,
      }),
    );
    expect(mocks.revalidar).toHaveBeenCalledWith(`/residentes/ficha/${residente}`);
    expect(mocks.revalidar).toHaveBeenCalledWith(
      "/residentes/[admissionId]/editar",
      "page",
    );
  },
);
it.each(["40001", "42501", "P0002", "otro"])(
  "conserva entradas y no reintenta ni revela errores: %s",
  async code => {
    mocks.rpc.mockResolvedValue({ data: null, error: { code, message: "DATO PRIVADO" } });
    const resultado = await guardarFamiliar(
      residente,
      contacto,
      version,
      inicial,
...
it("campos obligatorios e identificadores inválidos no llegan a la base", async () => {
  ...
  expect(
    (await guardarFamiliar(residente, contacto, version, inicial, new FormData())).errores
      .phone,
  ).toBeTruthy();
  expect(
    (await guardarFamiliar("invalido", contacto, version, inicial, formulario())).mensaje,
  ).toBeTruthy();
