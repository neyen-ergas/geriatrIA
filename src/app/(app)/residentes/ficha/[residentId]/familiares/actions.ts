...
export async function guardarFamiliar(
  ...
  if (
    ...
    !esIdFamiliar(contactoId) ||
    (version !== null &&
      (typeof version !== "string" || !Number.isFinite(Date.parse(version))))
  ) {
  ...
  try {
    ...
    const { data, error } = await cliente.rpc("save_family_contact", {
      ...
      p_relationship: valores.relationship,
      p_phone: valores.phone,
