...
export default async function RegistrosPage({
  ...
  return (
    <div className="space-y-5">
      ...
      <header>
        <h1 className="mt-3 text-2xl font-bold">{config.titulo}</h1>
      ...
      {!listado.registros.length ? (
        <Card className="p-6">No hay registros en esta selección.</Card>
      ) : (
        <ul className="space-y-4">
          {listado.registros.map(registro => (
            <li key={registro.id}>
              <Card className="p-5">
                <div className="mb-4">
                  <Badge>{estadoRegistro(registro, hoy)}</Badge>
                </div>
