...
vi.mock("@/lib/registros-residente-datos", () => ({
  listarRegistrosResidente: mocks.lista,
  obtenerRegistroResidente: mocks.registro,
  obtenerEstadiaDeRegistro: mocks.ingreso,
}));
...
const registro = {
  id: "registro",
  title: "Texto <script>",
  instructions: "Indicación ficticia",
  professional: "Profesional ficticio",
...
beforeEach(() => {
  ...
  mocks.persona.mockResolvedValue({
    residente: { first_name: "Persona", last_name: "Ficticia" },
  });
  mocks.lista.mockResolvedValue({ registros: [registro], total: 1, pagina: 1 });
  mocks.registro.mockResolvedValue(registro);
});
it.each([
  ...
  await expect(pagina()).rejects.toThrow("SIN_PERMISO");
...
it("Solo lectura consulta vigencia y texto escapado sin escrituras", async () => {
  ...
  const html = renderToStaticMarkup(
    await Listado({ params, searchParams: Promise.resolve({}) }),
  );
  expect(html).toContain("Vigencia finalizada");
...
it("registros archivados no muestran formulario de edición", async () => {
  ...
  expect(html).not.toContain("Guardar registro");
});
it("alta de pertenencia exige una estadía de la persona", async () => {
  mocks.ingreso.mockResolvedValue(null);
  await expect(
    Nuevo({
      params: Promise.resolve({ residentId: "persona", seccion: "pertenencias" }),
      searchParams: Promise.resolve({ ingreso: "ajeno" }),
    }),
...
it("formulario de medicación incluye pauta completa y profesional", async () => {
  const html = renderToStaticMarkup(
    await Nuevo({
      params: Promise.resolve({ residentId: "persona", seccion: "medicacion" }),
