...
export default async function FichaResidentePage({
  ...
  return (
    <div className="space-y-6">
      <Link href="/residentes" className={enlace}>
        ← Volver a Residentes
      </Link>
      <header>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-4">
          ...
          {puedeGestionar &&
            (ingresoActivoId ? (
              ...
              estadias.total > 0 && (
                <Link href={`/residentes/reingreso/${residente.id}`} className={enlace}>
                  Registrar reingreso
                </Link>
              )
            ))}
      ...
      <section aria-labelledby="familiares">
        <h2 id="familiares" className="text-lg font-semibold">
        ...
        {!contactos.total ? (
          <Card className="mt-4 p-5">No hay contactos registrados.</Card>
        ) : (
      ...
      <section aria-labelledby="estadias">
        <h2 id="estadias" className="text-lg font-semibold">
          Historial de estadías
        </h2>
        {!estadias.total ? (
          <Card className="mt-4 p-5">No hay estadías registradas.</Card>
        ) : (
