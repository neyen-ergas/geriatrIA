...
it("exige administración antes de procesar el formulario", async () => {
  ...
  await expect(guardarAcceso("cuenta", null, INICIAL, new FormData())).rejects.toThrow("DENEGADO");
...
it("conserva versión exacta, valida perfil y actualiza la navegación", async () => {
  const datos = new FormData(); datos.set("rol", "management"); datos.set("habilitado", "si");
  const version = "2026-09-13T12:00:00.123456Z";
  expect((await guardarAcceso("cuenta", version, INICIAL, datos)).ok).toBe(true);
  expect(mocks.rpc).toHaveBeenCalledWith("set_user_access", { p_user_id: "cuenta", p_role: "management", p_enabled: true, p_expected_updated_at: version });
...
it.each([
  ...
  const datos = new FormData(); datos.set("rol", "readonly"); datos.set("habilitado", "no");
