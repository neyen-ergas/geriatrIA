...
export async function buscarPersonaIngreso(
  ...
  try {
    ...
    if (!data)
      return {
        error: "No encontramos ese DNI. Completá el formulario de persona nueva.",
      };
    id = data.id;
  } catch {
    return { error: "No se pudo buscar la ficha. Intentá nuevamente." };
  }
...
export async function convertirPersonaNueva(
  consultaId: string,
  version: string,
  _anterior: EstadoFormularioIngreso,
  datos: FormData,
  ...
  try {
    previo = (await listarVinculosConsultas([consultaId]))[consultaId];
  } catch (error) {
    return { valores, errores: {}, mensaje: errorConversionConsulta(error) };
  }
  ...
  try {
    ...
    if (!residente)
      return {
        ...
        errores: {},
        mensaje:
          "La persona tiene un ingreso activo o no hay una baja anterior. Volvé a abrir la consulta para revisar el vínculo.",
      };
    const validacion = validarReingreso(
      datos,
      residente.lastAdmission.dischargedAt,
      hoyEnArgentina(),
    );
    if (!validacion.ok)
      return {
    ...
    const { data, error } = await supabase.rpc("convert_consultation_admission", {
      ...
      p_monthly_fee: validacion.datos.monthly_fee,
      p_due_day: validacion.datos.due_day,
