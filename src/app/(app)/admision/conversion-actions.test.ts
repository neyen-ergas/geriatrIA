...
import {
  buscarPersonaIngreso,
  convertirPersonaNueva,
  convertirReingreso,
} from "./[consultaId]/ingreso/actions";
...
vi.mock("@/lib/conversion-consulta-datos", () => ({
  listarVinculosConsultas: mocks.vinculos,
}));
vi.mock("@/lib/residentes-datos", () => ({
  obtenerResidenteParaReingreso: mocks.residente,
}));
...
describe.each(ACCIONES)("conversión desde una consulta", accion => {
  ...
  it("usa una sola transacción con la versión de la consulta y abre su cuenta", async () => {
    ...
    expect(mocks.rpc).toHaveBeenCalledExactlyOnceWith(
      ...
      expect.objectContaining({
        p_consultation_id: "consulta",
        p_expected_updated_at: VERSION,
        p_admitted_at: "2025-02-01",
        p_monthly_fee: 100,
        p_due_day: 10,
      }),
    );
  ...
  it.each(["40001", "23505", "23514", "23P01", "22023", "42501", "desconocido"])(
    "conserva valores sin reintentar ni exponer errores internos (%s)",
    async code => {
      mocks.rpc.mockResolvedValue({
        data: null,
        error: { code, message: "DATO PRIVADO" },
      });
  ...
  it("rechaza fechas futuras antes de escribir", async () => {
    ...
    datos.set("admitted_at", "2999-01-01");
...
it("busca DNI normalizado sin incluirlo en la URL", async () => {
  ...
  datos.set("dni", "12.345.678");
  await expect(buscarPersonaIngreso("consulta", { error: null }, datos)).rejects.toThrow(
    "REDIRECT",
  );
  expect(mocks.eq).toHaveBeenCalledWith("dni", "12345678");
  expect(mocks.redirigir).toHaveBeenCalledWith(
    "/admision/consulta/ingreso?residente=persona",
  );
});
