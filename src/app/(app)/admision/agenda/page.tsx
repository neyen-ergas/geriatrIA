  ...
  return (
    <div>
      ...
      <nav
        ...
        className="mt-5 flex flex-wrap items-center gap-4 text-sm font-medium text-sky-700"
      >
        {semana.anterior && (
          <Link href={enlaceAgenda(semana.anterior)}>← Semana anterior</Link>
        )}
        <Link href="/admision/agenda">Esta semana</Link>
        {semana.siguiente && (
          <Link href={enlaceAgenda(semana.siguiente)}>Semana siguiente →</Link>
        )}
      </nav>
      ...
      <p className="mt-5 text-sm text-slate-600">
        {etiquetaDiaAgenda(semana.inicio)} al {etiquetaDiaAgenda(semana.fin)} ·{" "}
