...
const mocks = vi.hoisted(() => ({ sesion: vi.fn(), visitas: vi.fn(), consulta: vi.fn(), noEncontrado: vi.fn() }));
...
vi.mock("../consulta-card", () => ({ ConsultaCard: () => <div>Detalle de la consulta</div> }));
const parametros = { searchParams: Promise.resolve({ semana: "2026-09-12" }) };
beforeEach(() => {
  vi.resetAllMocks();
  mocks.visitas.mockResolvedValue([]);
  mocks.noEncontrado.mockImplementation(() => { throw new Error("NOT_FOUND"); });
});
it.each([
...
it("un fallo de carga llega a la pantalla recuperable, sin dibujar una agenda vacía", async () => {
  mocks.visitas.mockRejectedValue(new Error("No se pudo cargar la disponibilidad de la semana."));
...
it("el detalle recupera la consulta y vuelve a la semana elegida", async () => {
  ...
  const html = renderToStaticMarkup(await ConsultaPage({ ...parametros, params: Promise.resolve({ consultaId: "consulta" }) }));
...
it("una consulta inexistente devuelve no encontrado", async () => {
  ...
  await expect(ConsultaPage({ ...parametros, params: Promise.resolve({ consultaId: "consulta" }) })).rejects.toThrow("NOT_FOUND");
});
