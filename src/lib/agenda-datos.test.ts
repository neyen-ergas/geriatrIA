      ...
      global: {
        fetch: async entrada => {
          ...
          const filtradas = filas.filter(
            f =>
              f.estado === url.searchParams.get("estado")?.slice(3) &&
              limites.every(l =>
                l.startsWith("gte.")
                  ? f.visita_fecha >= l.slice(4)
                  : f.visita_fecha <= l.slice(4),
              ),
          );
          return new Response(
            JSON.stringify(
              respuestaForzada ??
                filtradas.slice(0, Number(url.searchParams.get("limit"))),
            ),
            {
...
it("filtra en la base por estado y semana antes de limitar, incluso con más de mil cerradas", async () => {
  expect((await listarVisitasSemana("2026-09-07")).map(v => v.id)).toEqual([
    "consulta",
    "domingo",
  ]);
...
it("distingue una semana vacía de una carga fallida", async () => {
  ...
  await expect(listarVisitasSemana("2026-09-07")).rejects.toThrow(
    /^No se pudo cargar la disponibilidad de la semana\.$/,
  );
});
it.each([
  [visita, visita],
  [{ ...visita, visita_franja: null }],
  [{ ...visita, visita_fecha: "2026-09-14" }],
  Array.from({ length: 15 }, () => visita),
