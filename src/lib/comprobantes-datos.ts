...
type CargaComprobante = { ok: true; ruta: string | null } | { ok: false; error: string };
...
export async function cargarComprobante(
  ...
  try {
    ...
    if (errorSesion || !data?.claims.sub)
    ...
    if (error)
      return {
        ok: false,
        error:
          "No se pudo subir el comprobante. El pago no se registró; seleccioná el archivo de nuevo o continuá sin adjunto.",
      };
    return { ok: true, ruta };
  } catch {
    return {
      ok: false,
      error:
        "Se interrumpió la carga del comprobante. El pago no se registró; podés intentarlo nuevamente.",
    };
  }
}
