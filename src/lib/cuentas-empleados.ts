...
export function errorVinculo(error: unknown): string {
  ...
  const DUPLICADO = "23505",
    ...
    SIN_PERFIL = "P0002";
  if (codigo === DUPLICADO)
    return "Esta ficha ya tiene una cuenta vinculada. Volvé a cargarla para revisar el vínculo.";
  if (codigo === DESACTUALIZADO)
    return "La cuenta cambió. Volvé a cargar la ficha antes de vincular o desvincular.";
  if (codigo === SIN_PERFIL) return "Primero asigná un perfil a la cuenta desde Accesos.";
  if (codigo === RESTRICCION && mensaje === "employee_inactive")
    return "No se puede vincular una cuenta a un empleado dado de baja.";
  if (codigo === RESTRICCION && mensaje === "account_already_linked")
    return "La cuenta ya está vinculada a otra ficha. Revisá ese vínculo antes de cambiarlo.";
