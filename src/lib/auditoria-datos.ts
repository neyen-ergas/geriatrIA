...
export async function listarAuditoria(
  ...
  paginaParam: unknown,
): Promise<{ eventos: ResumenAuditoria[]; total: number; pagina: number }> {
  const cliente = await createClient();
  const consulta = (conteo: boolean) => {
    let seleccion = cliente
      .from("audit_events")
      .select(
        "id,table_name,record_id,action,occurred_at,actor_id,actor_label,record_label,changed_fields,origin",
    ...
    if (filtros.autor)
      seleccion =
        filtros.autor === "sin-usuario"
          ? seleccion.is("actor_id", null)
          : seleccion.eq("actor_id", filtros.autor);
    if (filtros.desde)
      seleccion = seleccion.gte("occurred_at", limiteFechaAuditoria(filtros.desde));
    if (filtros.hasta)
      seleccion = seleccion.lt("occurred_at", limiteFechaAuditoria(filtros.hasta, true));
    return seleccion;
  };
  const { count, error: errorConteo } = await consulta(true);
  if (errorConteo || count === null)
    throw new Error("No se pudo contar el historial. Intentá nuevamente.");
