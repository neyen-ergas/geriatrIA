        ...
        fetch: async (entrada, opciones) => {
          ...
          if (fallo === metodo)
            return new Response(
              JSON.stringify({
                code: "42501",
                message: "Información privada del servidor",
              }),
              { status: 403 },
            );
...
describe("lecturas de cuentas y cuotas", () => {
  ...
  it("cuenta sin descargar cuotas y aplica el mismo filtro de estadía a ambas lecturas", async () => {
    ...
    expect(
      solicitudes.every(
        s =>
          s.url.pathname.endsWith("/monthly_charge_balances") &&
          s.url.searchParams.get("admission_id") === `eq.${estadia}`,
      ),
    ).toBe(true);
  ...
  it.each([false, true])(
    "permite consultar cuentas activas o finalizadas: bajas=%s",
    async bajas => {
      respuesta = [];
      ...
      expect(
        solicitudes.every(
          s =>
            s.url.searchParams.get("discharged_at") ===
            (bajas ? "not.is.null" : "is.null"),
        ),
      ).toBe(true);
      expect(solicitudes[1].url.searchParams.get("order")).toBe(
        "residents(last_name).asc,residents(first_name).asc,admitted_at.desc,id.asc",
      );
    },
  );
  ...
  it("distingue una cuenta vacía de un error", async () => {
    respuesta = [];
    total = 0;
    expect(await listarCuotas(estadia, "99")).toEqual({
      cuotas: [],
      total: 0,
      pagina: 1,
    });
  ...
  it.each(["HEAD", "GET"] as const)(
    "no devuelve saldos ni detalles privados ante error %s",
    async metodo => {
      fallo = metodo;
      await expect(listarCuotas(estadia, "1")).rejects.toThrow(
        metodo === "HEAD"
          ? "No se pudieron contar las cuotas."
          : "No se pudieron leer las cuotas.",
      );
  ...
  it.each([{ balance: null }, { payment_status: "desconocido" }])(
    "rechaza datos incompletos sin inventar un saldo o estado",
    async cambio => {
