...
export type ResumenEmpleado = Pick<Empleado, "id" | "first_name" | "last_name" | "dni" | "job_title" | "hired_at" | "terminated_at">;

export async function listarEmpleados(bajas: boolean, paginaParam: unknown): Promise<{ empleados: ResumenEmpleado[]; total: number; pagina: number }> {
  const supabase = await createClient();
  const consulta = (conteo: boolean) => {
    const seleccion = supabase.from("employees").select("id, first_name, last_name, dni, job_title, hired_at, terminated_at", conteo ? { count: "exact", head: true } : {});
    return bajas ? seleccion.not("terminated_at", "is", null) : seleccion.is("terminated_at", null);
  };
  ...
  const { data, error } = await consulta(false).order("last_name").order("first_name").order("id")
