      ...
      global: {
        fetch: async (entrada, opciones) => {
          ...
          return new Response(
            metodo === "HEAD"
              ? null
              : JSON.stringify([{ id: pagoId, voided_at: "2026-09-10T12:00:00Z" }]),
...
it("pagina todos los pagos de la cuota, incluidos anulados, sin truncar el conteo", async () => {
  ...
  expect(
    solicitudes.every(s => s.url.searchParams.get("monthly_charge_id") === "eq.cuota"),
  ).toBe(true);
  ...
  expect(solicitudes[1].url.searchParams.get("order")).toBe(
    "paid_on.desc,created_at.desc,id.desc",
  );
...
it("no presenta un error como listado vacío", async () => {
  fallo = true;
  await expect(listarMovimientos("cuota", "1")).rejects.toThrow(
    "No se pudieron contar los pagos.",
  );
