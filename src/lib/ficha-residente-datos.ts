...
export async function obtenerFichaResidente(
  residenteId: string, paginaEstadias: unknown, paginaContactos: unknown,
): Promise<FichaResidente | null> {
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i
    .test(residenteId)) return null;
  ...
  const [conteoEstadias, conteoContactos, activo] = await Promise.all([
    cliente.from("admissions").select("id", { count: "exact", head: true })
      .eq("resident_id", residenteId),
    cliente.from("family_contacts").select("id", { count: "exact", head: true })
      .eq("resident_id", residenteId),
    cliente.from("admissions").select("id").eq("resident_id", residenteId)
      .is("discharged_at", null).maybeSingle(),
  ]);
  if (conteoEstadias.error || conteoEstadias.count === null
    || conteoContactos.error || conteoContactos.count === null || activo.error) {
  ...
  const [historial, familiares] = await Promise.all([
    cliente.from("admissions")
      .select("id,admitted_at,discharged_at,discharge_reason,room,monthly_fee,currency,due_day,administrative_notes")
      .eq("resident_id", residenteId)
      .order("admitted_at", { ascending: false }).order("id", { ascending: false })
      .range(inicioEstadias, inicioEstadias + REGISTROS_POR_PAGINA - 1),
    cliente.from("family_contacts")
      .select("id,first_name,last_name,relationship,phone,is_emergency_contact,is_payment_responsible,notes")
      .eq("resident_id", residenteId)
  ...
  return {
    residente, ingresoActivoId: activo.data?.id ?? null,
    estadias: {
      filas: historial.data ?? [], total: conteoEstadias.count, pagina: estadias,
    },
    contactos: {
      filas: familiares.data ?? [], total: conteoContactos.count, pagina: contactos,
    },
  };
}
