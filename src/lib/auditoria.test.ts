...
import { enlaceAuditoria, leerFiltrosAuditoria, limiteFechaAuditoria, regresoAuditoria } from "./auditoria";
import { formatearValorAuditoria } from "./auditoria-campos";
import { puedeVerSeccion } from "./permisos";

it.each([
  { tabla: "auth.users" }, { accion: "truncate" }, { autor: "x,or(role.eq.admin)" },
  { desde: "2025-02-29" }, { hasta: "2026-04-31" }, { desde: "2026-09-02", hasta: "2026-09-01" },
  { tabla: ["payments", "employees"] }, { registro: "84000000-0000-4000-8000-000000000001" },
...
it("admite fechas reales y límites inclusivos del día argentino", () => {
  expect(leerFiltrosAuditoria({ desde: "2024-02-29", hasta: "2024-02-29", autor: "sin-usuario" }).error).toBeNull();
...
it("conserva filtros y página al volver, sin admitir destinos externos", () => {
  const enlace = enlaceAuditoria({ tabla: "payments", autor: "sin-usuario", desde: "2026-09-01" }, 4);
  expect(regresoAuditoria(enlace)).toBe(enlace);
  for (const entrada of ["https://evil.invalid", "//evil.invalid", "/auditoria/../accesos", [enlace], "/auditoria?tabla=payments&tabla=employees"])
