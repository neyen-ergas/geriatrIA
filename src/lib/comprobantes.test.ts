...
it.each([
  ...
  "acepta la firma de %s sin confiar en el nombre",
  async (tipo, firma, extension) => {
    const archivo = new File([new Uint8Array(firma)], "nombre-privado.exe", {
      type: tipo,
    });
    expect(await validarComprobante(archivo)).toEqual({
      ok: true,
      datos: { archivo, extension },
    });
  },
);
it("rechaza archivos vacíos, formato disfrazado y archivos grandes", async () => {
  for (const archivo of [
    ...
    new File([new Uint8Array(MAX_COMPROBANTE_BYTES + 1)], "grande.jpg", {
      type: "image/jpeg",
    }),
    "ruta/inventada.pdf",
  ]) {
...
it("no acepta rutas de otra cuota ni traversal al descargar", () => {
  const ruta =
    "11111111-1111-4111-8111-111111111111/estadia/cuota/22222222-2222-4222-8222-222222222222.pdf";
  expect(perteneceComprobante(ruta, "estadia", "cuota")).toBe(true);
  expect(perteneceComprobante(ruta, "estadia", "otra")).toBe(false);
  expect(
    perteneceComprobante(ruta.replace("cuota/", "cuota/../"), "estadia", "cuota"),
  ).toBe(false);
});
