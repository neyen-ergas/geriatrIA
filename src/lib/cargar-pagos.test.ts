...
describe("carga de cuotas y pagos", () => {
  ...
  ] as const)("ajusta el vencimiento de %s día %s", (mes, dia, esperado) => {
    expect(vencimientoSugerido(mes, dia)).toBe(esperado);
  });
  it("crea argumentos con el importe argentino y el primer día del período", () => {
    expect(validarCargaCuota(valores, estadia)).toEqual({
      ok: true,
      datos: {
        p_admission_id: "estadia",
        p_period: "2026-02-01",
  ...
  it.each(["2025-12", "2026-04", "2026-13"])(
    "rechaza mes fuera de estadía o inválido: %s",
    periodo => {
  ...
  it("acepta los meses de ingreso y baja sin prorratear", () => {
    for (const periodo of ["2026-01", "2026-03"]) {
      expect(
        validarCargaCuota({ ...valores, periodo, vencimiento: `${periodo}-28` }, estadia)
          .ok,
      ).toBe(true);
    }
  });
  ...
  it("acepta pago parcial o total y rechaza exceso, cuota anulada y fecha futura", () => {
    ...
    expect(
      validarCargaPago({ ...valores, importe: "1500" }, cuota, "2026-02-10").ok,
    ).toBe(true);
    expect(
      validarCargaPago({ ...valores, importe: "1500,01" }, cuota, "2026-02-10").ok,
    ).toBe(false);
    expect(
      validarCargaPago(valores, { ...cuota, payment_status: "cancelled" }, "2026-02-10")
        .ok,
    ).toBe(false);
  ...
  it("valida el medio y exige aclaración para otro", () => {
    ...
    expect(
      validarCargaPago(
        { ...valores, medio: "other", notas: "Cheque" },
        cuota,
        "2026-02-10",
      ).ok,
    ).toBe(true);
