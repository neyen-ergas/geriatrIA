...
it("delimita meses cortos, bisiestos y diciembre sin incluir el mes siguiente", () => {
  expect(limitesMesVencimientos("2024-02")).toEqual({
    inicio: "2024-02-01",
    fin: "2024-02-29",
  });
...
it("conserva mes y filtro al navegar y vuelve a primera página sin perderlos", () => {
  expect(enlaceVencimientos("2026-09", true, 2)).toBe(
    "/contabilidad/vencimientos?mes=2026-09&estado=vencidas&pagina=2",
  );
  expect(enlaceVencimientos("2026-09", true)).toBe(
    "/contabilidad/vencimientos?mes=2026-09&estado=vencidas",
  );
});
