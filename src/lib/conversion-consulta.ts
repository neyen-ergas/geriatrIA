export function errorConversionConsulta(error: unknown): string {
  ...
  if (codigo === VERSION_CAMBIADA) return "La consulta cambió. Volvé a cargar antes de registrar el ingreso.";
  if (codigo === DUPLICADO) return "Ya existe ese DNI o un ingreso activo. Buscá la ficha existente antes de continuar.";
  ...
  if (codigo === "22023") return "Esta consulta no está disponible para registrar un ingreso. Volvé a cargarla.";
  if (codigo === "42501") return "Tu sesión no permite registrar este ingreso. Volvé a iniciar sesión.";
