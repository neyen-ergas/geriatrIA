...
export type FamiliarEditable = Pick<
...
export function leerFamiliar(datos: FormData): ValoresFamiliar {
  ...
  return {
    ...
    relationship: texto("relationship"),
    phone: texto("phone"),
    notes: texto("notes"),
...
export function errorFamiliar(error: unknown): string {
  ...
  const DESACTUALIZADO = "40001",
    SIN_ACCESO = "42501",
    INEXISTENTE = "P0002";
  if (codigo === DESACTUALIZADO)
    return "El contacto cambió. Volvé a la ficha y abrilo nuevamente antes de guardar.";
  if (codigo === SIN_ACCESO)
    return "Tu sesión no permite guardar contactos. Volvé a iniciar sesión.";
  if (codigo === INEXISTENTE)
    return "El residente o contacto ya no está disponible. Volvé a la ficha.";
