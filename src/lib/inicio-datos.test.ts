  ...
  const cliente = crearCliente<Database>("https://supabase.invalid", "clave-ficticia", {
    ...
    global: {
      fetch: async entrada => {
        ...
        if (tabla === "monthly_charge_balances" && fallarCuotas)
          return new Response('{"message":"dato privado"}', { status: 403 });
        const total =
          tabla === "consulta" && url.searchParams.get("estado") === "eq.nuevo"
            ? 1255
            : 1;
        const filas =
          tabla === "consulta"
            ...
            : tabla === "admissions"
              ? [
                  {
                    id: "ingreso",
                    admitted_at: "2026-12-30",
                    discharged_at: "2026-12-31",
                    residents: persona,
                  },
                ]
              : [
                  {
                    ...
                    balance: 60,
                    currency: "ARS",
                    admissions: { residents: persona },
                  },
                ];
        return new Response(
          JSON.stringify(filas.slice(0, Number(url.searchParams.get("limit")))),
          {
            headers: {
              "Content-Type": "application/json",
              ...(!omitirConteo ? { "Content-Range": `0-4/${total}` } : {}),
            },
          },
        );
      },
...
it("hoy y mañana cruzan el año y los ingresos abarcan siete días civiles", async () => {
  const bloques = await obtenerInicio("2026-12-31");
  const visitas = solicitudes.filter(
    u => u.searchParams.get("estado") === "eq.visita_agendada",
  );
  expect(visitas.map(u => u.searchParams.get("visita_fecha"))).toEqual([
    "eq.2026-12-31",
    "eq.2027-01-01",
  ]);
  const ingresos = solicitudes.find(u => u.pathname.endsWith("/admissions"))!;
  expect(ingresos.searchParams.getAll("admitted_at")).toEqual([
    "gte.2026-12-25",
    "lte.2026-12-31",
  ]);
...
it("las vencidas abarcan todos los meses y enlazan a su cuota exacta", async () => {
  ...
  expect(bloques[3].resumen?.elementos[0].href).toBe(
    "/contabilidad/ingreso/cuotas/cuota",
  );
});
