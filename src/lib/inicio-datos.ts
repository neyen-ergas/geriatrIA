...
export async function obtenerInicio(hoy: string): Promise<BloqueInicio[]> {
  ...
  const bloques = [
    {
      titulo: "Consultas sin llamar",
      descripcion: "Las más antiguas primero",
      href: "/admision?estado=nuevo",
      cargar: sinLlamar,
    },
    {
      titulo: "Visitas de hoy",
      descripcion: formatearFechaPago(hoy),
      href: enlaceAgenda(semanaAgenda(hoy, hoy).inicio),
      cargar: () => visitasDia(hoy),
    },
    {
      titulo: "Visitas de mañana",
      descripcion: formatearFechaPago(manana),
      href: enlaceAgenda(semanaAgenda(manana, manana).inicio),
      cargar: () => visitasDia(manana),
    },
    {
      titulo: "Cuotas vencidas",
      descripcion: "Con saldo pendiente, de todos los meses",
      href: "/contabilidad/vencimientos?alcance=todas",
      cargar: cuotasVencidas,
    },
    {
      titulo: "Ingresos recientes",
      descripcion: `Últimos ${DIAS_INGRESOS_RECIENTES} días, incluidos reingresos`,
      href: "/residentes",
      cargar: () => ingresosRecientes(desde, hoy),
    },
  ];
  ...
  return bloques.map(({ cargar: _cargar, ...bloque }, i) => {
    ...
    return {
      ...bloque,
      resumen: resultado.status === "fulfilled" ? resultado.value : null,
    };
  });
}
...
async function sinLlamar(): Promise<Resumen> {
  const { data, error, count } = await (
    ...
    .order("creado_en")
    .order("id")
    .limit(ELEMENTOS_POR_BLOQUE);
  if (error || !data || count === null)
    throw new Error("No se pudieron leer las consultas pendientes.");
  return {
    total: count,
    elementos: data.map(c => ({
      id: c.id,
      titulo: c.nombre,
      detalle: c.telefono,
      href: `/admision/${c.id}`,
...
async function visitasDia(fecha: string): Promise<Resumen> {
  const { data, error, count } = await (
    ...
    .eq("visita_fecha", fecha)
    .order("visita_franja")
    .order("id")
    .limit(ELEMENTOS_POR_BLOQUE);
  if (error || !data || count === null)
    throw new Error("No se pudieron leer las visitas.");
  return {
    total: count,
    elementos: data.map(c => {
      ...
      return {
        id: c.id,
        titulo: c.nombre,
        detalle: `${FRANJAS[c.visita_franja]} · ${c.telefono}`,
...
async function cuotasVencidas(): Promise<Resumen> {
  ...
  const { data, error, count } = await supabase
    ...
    .select(
      "id, admission_id, due_date, balance, currency, admissions!inner(residents!inner(first_name, last_name))",
      { count: "exact" },
    )
    .eq("is_overdue", true)
    .gt("balance", 0)
    ...
    .limit(ELEMENTOS_POR_BLOQUE);
  if (error || !data || count === null)
    throw new Error("No se pudieron leer las cuotas vencidas.");
  return {
    total: count,
    elementos: data.map(c => {
      if (!c.id || !c.admission_id || !c.due_date || c.balance === null || !c.currency)
        throw new Error("La cuota no es válida.");
      const persona = c.admissions.residents;
      return {
        id: c.id,
        titulo: `${persona.last_name}, ${persona.first_name}`,
...
async function ingresosRecientes(desde: string, hasta: string): Promise<Resumen> {
  ...
  const { data, error, count } = await supabase
    .from("admissions")
    .select("id, admitted_at, discharged_at, residents!inner(first_name, last_name)", {
      count: "exact",
    })
    .gte("admitted_at", desde)
    .lte("admitted_at", hasta)
    .order("admitted_at", { ascending: false })
    .order("id", { ascending: false })
    .limit(ELEMENTOS_POR_BLOQUE);
  if (error || !data || count === null)
    throw new Error("No se pudieron leer los ingresos recientes.");
  return {
    total: count,
    elementos: data.map(c => ({
      id: c.id,
