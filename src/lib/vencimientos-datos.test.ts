...
const base = {
  ...
  admissions: {
    ...
    discharged_at: "2026-09-02",
    residents: { first_name: "Persona ficticia", last_name: "Prueba", dni: "TEST" },
  },
};
const filas = [
  ...Array.from({ length: 1255 }, (_, indice) => ({
    ...base,
    id: String(indice).padStart(5, "0"),
    due_date: indice % 2 ? "2026-09-10" : "2026-09-01",
    is_overdue: indice % 2 === 0,
  })),
    ...
    crearSupabase<Database>("https://supabase.invalid", "clave-ficticia", {
      ...
      global: {
        fetch: async (entrada, opciones) => {
          ...
          if (fallo === metodo)
            return new Response(JSON.stringify({ message: "dato privado" }), {
              status: 403,
            });
          ...
          for (const filtro of parametros.getAll("due_date")) {
            ...
            resultado = resultado.filter(f =>
              filtro.startsWith("gte.") ? f.due_date >= fecha : f.due_date <= fecha,
            );
          }
          if (parametros.get("balance") === "gt.0")
            resultado = resultado.filter(f => f.balance > 0);
          if (parametros.get("cancelled_at") === "is.null")
            resultado = resultado.filter(f => f.cancelled_at === null);
          if (parametros.get("is_overdue") === "eq.true")
            resultado = resultado.filter(f => f.is_overdue);
          ...
          resultado = resultado.slice(
            inicio,
            inicio + Math.min(Number(parametros.get("limit") ?? 1000), 1000),
          );
          return new Response(metodo === "HEAD" ? null : JSON.stringify(resultado), {
            headers: conteoAusente
              ? {}
              : { "content-range": `0-${resultado.length - 1}/${total}` },
          });
        },
      },
    }),
...
it("filtra vencidas en conteo y filas, manteniendo cuotas de estadías finalizadas", async () => {
  ...
  expect(
    datos.vencimientos.every(v => v.cuota.is_overdue && v.estadia.discharged_at),
  ).toBe(true);
  expect(solicitudes.every(s => s.url.searchParams.get("is_overdue") === "eq.true")).toBe(
    true,
  );
  expect(
    solicitudes.every(s => !s.url.searchParams.has("admissions.discharged_at")),
  ).toBe(true);
});
it.each(["HEAD", "GET"] as const)(
  "presenta error seguro si falla %s, no un listado vacío",
  async metodo => {
    fallo = metodo;
    await expect(listarVencimientos("2026-09", false, "1")).rejects.toThrow(
      metodo === "HEAD"
        ? "No se pudieron contar los vencimientos."
        : "No se pudieron leer los vencimientos.",
    );
  },
);
...
it("muestra vacío cuando no existen cuotas del mes", async () => {
  expect(await listarVencimientos("2026-07", false, "5")).toEqual({
    vencimientos: [],
    total: 0,
    pagina: 1,
  });
