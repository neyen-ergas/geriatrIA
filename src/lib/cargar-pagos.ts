...
type Validacion<Datos> =
  { ok: true; datos: Datos } | { ok: false; errores: Record<string, string> };
...
export function leerCargaPago(formData: FormData): Record<string, string> {
  return Object.fromEntries(
    ["periodo", "vencimiento", "importe", "fecha", "medio", "referencia", "notas"].map(
      campo => {
...
export function vencimientoSugerido(periodo: string, dia: number): string {
  if (!esFechaValida(`${periodo}-01`) || !Number.isInteger(dia) || dia < 1 || dia > 31)
    return "";
...
export function validarCargaCuota(
  ...
  if (!esFechaValida(periodo)) errores.periodo = "Elegí un mes válido.";
    ...
    ultimoDia < estadia.admitted_at ||
    (estadia.discharged_at && periodo > estadia.discharged_at)
  ) {
    errores.periodo = "El mes debe corresponder a esta estadía.";
  }
  if (
    !esFechaValida(valores.vencimiento) ||
    valores.vencimiento < periodo ||
    valores.vencimiento > ultimoDia
  ) {
  ...
  return {
    ok: true,
    datos: {
      p_admission_id: estadia.id,
      p_period: periodo,
      p_due_date: valores.vencimiento,
      p_amount_due: importe,
...
export function validarCargaPago(
  ...
  return {
    ok: true,
    datos: {
      p_monthly_charge_id: cuota.id,
      p_amount: importe,
      p_paid_on: valores.fecha,
      p_payment_method: valores.medio,
...
export function errorCargaPago(
  error: unknown,
): Pick<EstadoCargaPago, "mensaje" | "bloqueado"> {
  const codigo =
  ...
  if (codigo === CHECK_VIOLATION)
    return {
      mensaje:
        "La cuota o la estadía cambió, o el importe supera el saldo. Volvé a la cuenta para revisar los datos.",
    };
  if (codigo === INVALID_PARAMETER)
    return { mensaje: "Revisá el importe, las fechas y el medio de pago." };
  if (codigo === NOT_FOUND)
    return { mensaje: "La cuota o la estadía ya no está disponible." };
  if (codigo === INSUFFICIENT_PRIVILEGE)
    return {
      mensaje: "Tu sesión no permite registrar este movimiento. Volvé a iniciar sesión.",
    };
  return {
    mensaje:
      "No se pudo confirmar el resultado. Volvé a la cuenta y revisá si el movimiento quedó registrado antes de volver a cargarlo.",
    bloqueado: true,
  };
}
