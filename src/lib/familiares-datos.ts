...
export async function obtenerFormularioFamiliar(
  ...
  if (
    ...
    (contactoId !== undefined && !esIdFamiliar(contactoId))
  )
    return null;
  ...
  const { data: contacto, error: errorContacto } = await cliente
    .from("family_contacts")
    .select(
      "id,resident_id,first_name,last_name,relationship,phone,notes,is_emergency_contact,is_payment_responsible,updated_at",
    )
    .eq("id", contactoId)
