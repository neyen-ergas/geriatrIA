...
type SaldoCuota = Pick<
  Tables<"monthly_charge_balances">,
  | "id"
  | "period"
  | "due_date"
  | "amount_due"
...
>;
export type Cuota = {
  [Campo in keyof Omit<SaldoCuota, "cancelled_reason" | "payment_status">]: NonNullable<
    SaldoCuota[Campo]
  >;
...
export function esCuota(valor: SaldoCuota): valor is Cuota {
  return (
    ...
    typeof valor.due_date === "string" &&
    typeof valor.currency === "string" &&
    typeof valor.is_overdue === "boolean" &&
    typeof valor.amount_due === "number" &&
    Number.isFinite(valor.amount_due) &&
    typeof valor.paid_amount === "number" &&
    Number.isFinite(valor.paid_amount) &&
    typeof valor.balance === "number" &&
    Number.isFinite(valor.balance) &&
