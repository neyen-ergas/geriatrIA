...
it("explica cómo resolver la baja cuando la cuenta sigue habilitada", () => {
  expect(errorEmpleado({ code: "23514", message: "employee_access_enabled" })).toContain(
...
it("normaliza DNI y admite contactos y nacimiento vacíos", () => {
  ...
  expect(resultado).toMatchObject({
    ok: true,
    datos: { p_dni: "12345678", p_birth_date: undefined, p_email: undefined },
  });
});
it.each([
  ["first_name", ""],
...
it.each(["2024-12-31", "2999-01-01", "2026-02-30"])("rechaza fecha de baja %s", fecha => {
  expect(
    validarBajaEmpleado(
      { ...base(), terminated_at: fecha, termination_reason: "Fin ficticio" },
      "2025-01-01",
      "2026-09-13",
    ),
  ).toHaveProperty("terminated_at");
});
it("permite baja el mismo día del alta y exige motivo", () => {
  expect(
    validarBajaEmpleado(
      { ...base(), terminated_at: "2025-01-01", termination_reason: "Fin ficticio" },
      "2025-01-01",
      "2026-09-13",
    ),
  ).toEqual({});
