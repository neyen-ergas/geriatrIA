...
export const ETIQUETAS_TABLA: Record<TablaAuditoria, string> = {
  consulta: "Consultas y visitas",
  residents: "Residentes",
  family_contacts: "Familiares",
  admissions: "Estadías",
  monthly_charges: "Cuotas",
  payments: "Pagos",
  employees: "Empleados",
  user_access: "Accesos y cuentas",
  consultation_admissions: "Conversión a ingreso",
  resident_documents: "Documentos del residente",
  medical_indications: "Indicaciones médicas",
...
export type ResumenAuditoria = Omit<
  EventoAuditoria,
...
export function leerFiltrosAuditoria(
  parametros: Record<string, string | string[] | undefined>,
): { filtros: FiltrosAuditoria; error: string | null } {
  ...
  for (const clave of [
    ...
    if (typeof valor !== "string")
      return { filtros, error: "Cada filtro debe tener un solo valor." };
    if (clave === "tabla") {
      if (!esTablaAuditoria(valor))
        return { filtros, error: "Elegí una sección válida." };
      filtros.tabla = valor;
    } else if (clave === "accion") {
      if (!esAccionAuditoria(valor))
        return { filtros, error: "Elegí un tipo de cambio válido." };
      filtros.accion = valor;
    } else if (clave === "autor" || clave === "registro") {
      if (
        !esIdentificadorAuditoria(valor) &&
        !(clave === "autor" && valor === "sin-usuario")
      )
        return { filtros, error: "El autor o registro del filtro no es válido." };
      filtros[clave] = valor;
    } else {
      if (!esFechaAuditoria(valor))
        return { filtros, error: "Revisá las fechas del filtro." };
      filtros[clave] = valor;
    }
  }
  if (filtros.desde && filtros.hasta && filtros.desde > filtros.hasta)
    return { filtros, error: "La fecha desde no puede ser posterior a la fecha hasta." };
  if (filtros.registro && !filtros.tabla)
    return { filtros, error: "Elegí la sección del registro que querés consultar." };
...
export function regresoAuditoria(valor: unknown): string {
  ...
  return enlaceAuditoria(
    filtros,
    Number.isSafeInteger(pagina) && pagina > 0 ? pagina : 1,
  );
}
...
export function valoresAuditoria(
  valor: EventoAuditoria["new_values"],
