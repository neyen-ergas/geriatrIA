      ...
      global: {
        fetch: async (entrada, opciones) => {
          ...
          if (tabla === "residents") {
            ...
            filas = Array.from({ length: 1205 }, (_, id) => ({ id: String(id) })).slice(
              inicio,
              inicio + Number(url.searchParams.get("limit") ?? 1000),
            );
          }
...
it("pagina ambos historiales completos y consulta el ingreso activo aparte", async () => {
  ...
  for (const url of solicitudes) {
    expect(
      url.searchParams.get(url.pathname.endsWith("/residents") ? "id" : "resident_id"),
    ).toBe(`eq.${residenteId}`);
  }
...
it.each(["residents", "admissions", "family_contacts", "conteo", "sin-conteo"])(
  "no convierte errores en datos vacíos ni expone el mensaje de base: %s",
  async modo => {
    fallo = modo;
    await expect(obtenerFichaResidente(residenteId, "1", "1")).rejects.toThrow(
      /^No se pud/,
    );
  },
);
