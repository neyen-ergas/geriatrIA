...
beforeEach(() => {
  ...
  vi.mocked(crearClienteSesion).mockResolvedValue(createClient<Database>("https://supabase.invalid", "clave-ficticia", {
    auth: { persistSession: false }, global: { fetch: async (entrada, opciones) => {
      ...
      if (estado?.startsWith("eq.")) resultado = resultado.filter(f => f.estado === estado.slice(3));
      if (estado?.startsWith("in.")) resultado = resultado.filter(f => ["nuevo", "contactado"].includes(f.estado));
      ...
      resultado = resultado.slice(inicio, inicio + Math.min(Number(url.searchParams.get("limit") ?? 1000), 1000));
      return new Response(opciones?.method === "HEAD" ? null : JSON.stringify(resultado), {
...
it("busca en conteos y filas sin limitar a mil, manteniendo estado y página", async () => {
  ...
  expect(conteo.nuevo).toBe(1205); expect(conteo.ingreso).toBe(0);
...
it("las candidatas excluyen cerradas y conservan búsqueda al ajustar una página fuera de rango", async () => {
  ...
  expect(solicitudes.every(u => u.searchParams.get("estado") === "in.(nuevo,contactado)")).toBe(true);
  expect(solicitudes.every(u => !u.searchParams.get("select")?.includes("notas"))).toBe(true);
});
