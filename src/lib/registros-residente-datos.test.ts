...
import {
  listarRegistrosResidente,
  obtenerRegistroResidente,
  obtenerEstadiaDeRegistro,
} from "./registros-residente-datos";
    ...
    crearSupabase<Database>("https://supabase.invalid", "ficticia", {
      ...
      global: {
        fetch: async (entrada, opciones) => {
          ...
          const filas = Array.from({ length: 1205 }, (_, id) => ({
            id: String(id),
...
it.each(SECCIONES_REGISTRO)(
  "%s filtra persona/archivo en conteo y página, con más de mil filas",
  async seccion => {
    const resultado = await listarRegistrosResidente(persona, seccion, true, "999");
    expect(resultado).toMatchObject({ pagina: 25, total: 1205 });
    expect(resultado.registros).toHaveLength(5);
    expect(
      urls.every(
        u =>
          u.searchParams.get("resident_id") === `eq.${persona}` &&
          u.searchParams.get("archived_at") === "not.is.null",
      ),
    ).toBe(true);
...
it("errores no equivalen a historial vacío e ids inválidos no consultan", async () => {
  ...
  expect(await obtenerEstadiaDeRegistro(persona, "invalido")).toBeNull();
  expect(urls).toHaveLength(0);
  fallo = true;
  await expect(
    listarRegistrosResidente(persona, "documentos", false, "1"),
  ).rejects.toThrow("No se pudo contar el historial.");
});
